...
def create_material_file(material_name):
    ...
    if not mark("Refraction") and not mark("WaterNormal"):
        material_file_content += """
            "LightmappedGeneric"
            {
                "%keywords" "gm_post_ussr"
        """    
    ...
    if mark("WaterNormal"):
        material_file_content += """
            ...
            "$REFLECTBLENDFACTOR" "0.0"
            "$nofresnel" "1"
            "%tooltexture" \"""" + cd_materials + material_name + "_" + mark_to_postfix["WaterNormal"] + """"
            ...
            "$bumpmap" \"""" + cd_materials + material_name + "_" + mark_to_postfix["WaterNormal"] + """"
            "$normalmap" \"""" + cd_materials + material_name + "_" + mark_to_postfix["WaterNormal"] + """"

            "%compilewater" 1

            "$surfaceprop" "water"
            "$bottommaterial" \"""" + cd_materials + material_name + "_BOTTOM" + """"
        ...
        with open(path_to_export_textures + "/" + material_name + "_BOTTOM" + ".vmt", "w") as f:
            f.write("""
                "Water" { 
                    "%keywords" "gm_post_ussr"
                    "Water_DX81"
